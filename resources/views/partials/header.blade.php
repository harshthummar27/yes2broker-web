...
<div class="bg-y2b-primary text-white text-xs xl:text-sm">
    <div class="w-full max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8 py-2 flex items-center justify-between gap-3">
        <div class="flex items-center gap-3 sm:gap-4 xl:gap-6 shrink-0">
            <a href="mailto:{{ config('site.email') }}" class="flex items-center gap-1.5 hover:text-y2b-light transition">
                <svg class="w-3.5 h-3.5 xl:w-4 xl:h-4" fill="currentColor" viewBox="0 0 512 512"><path d="M464 64H48C21.49 64 0 85.49 0 112v288c0 26.51 21.49 48 48 48h416c26.51 0 48-21.49 48-48V112c0-26.51-21.49-48-48-48zm0 48v40.805c-22.422 18.259-58.168 46.651-134.587 106.49-16.841 13.247-50.201 45.072-73.413 44.701-23.208.375-56.579-31.459-73.413-44.701C106.18 199.465 70.425 171.067 48 152.805V112h416zM48 400V214.398c22.914 18.251 55.409 43.862 104.938 82.646 21.857 17.205 60.134 55.186 103.062 54.955 42.717.231 80.509-37.199 103.053-54.947 49.528-38.783 82.032-64.401 104.947-82.653V400H48z"/></svg>
            ...
            @foreach(config('site.social') as $platform => $url)
                <a href="{{ $url }}" target="_blank" rel="noopener" class="hover:text-y2b-light transition">
                    @if($platform === 'facebook')
                        <svg class="w-3.5 h-3.5 xl:w-4 xl:h-4" fill="currentColor" viewBox="0 0 512 512"><path d="M504 256C504 119 393 8 256 8S8 119 8 256c0 123.78 90.69 226.38 209.25 245V327.69h-63V256h63v-54.64c0-62.15 37-96.48 93.67-96.48 27.14 0 55.52 4.84 55.52 4.84v61h-31.28c-30.8 0-40.41 19.12-40.41 38.73V256h68.78l-11 71.69h-57.78V501C413.31 482.38 504 379.78 504 256z"/></svg>
...
<<<<<<< HEAD
    <div class="w-full max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-10 py-3.5 xl:py-4 flex items-center justify-between gap-4">
        ...
        <a href="{{ route('home') }}" class="shrink-0 flex items-center">
            <img src="{{ site_asset(config('site.logo')) }}" alt="{{ config('site.name') }}" class="h-11 sm:h-12 xl:h-14 2xl:h-16 w-auto object-contain">
        </a>
...
>>>>>>> c16417786fbd38c92fdbb398edd1e66b4a1545f0
            @foreach([
                ...
                <a href="{{ route($item['route']) }}"
                   class="nav-link hover:text-y2b-primary transition {{ request()->routeIs($item['route']) ? 'text-y2b-primary font-bold active' : '' }}">
                    {{ $item['label'] }}
                </a>
            @endforeach
        </nav>
...
>>>>>>> c16417786fbd38c92fdbb398edd1e66b4a1545f0
    ...
    <div id="mobile-menu" class="hidden lg:hidden border-t bg-white">
        <nav class="w-full px-4 py-4 flex flex-col gap-3 text-base font-semibold">
            @foreach([
                ['route' => 'home', 'label' => 'Home'],
                ['route' => 'about', 'label' => 'About Us'],
                ['route' => 'properties.index', 'label' => 'All Properties'],
                ...
                <a href="{{ route($item['route']) }}" class="py-2 border-b border-gray-100 hover:text-y2b-primary {{ request()->routeIs($item['route']) ? 'text-y2b-primary font-bold' : '' }}">
                    {{ $item['label'] }}
                </a>
            @endforeach
            <button type="button" data-open-consultation
                    class="mt-2 w-full bg-y2b-primary text-white text-center py-3 rounded-lg font-semibold cursor-pointer">
                INQUIRE NOW
            </button>
        </nav>
    </div>
